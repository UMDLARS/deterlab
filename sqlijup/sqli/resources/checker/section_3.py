...
def main():
    ...
    if (os.path.exists("/var/www/html/php_practice.php")):
        ...
        for line in sourceCode:
            ...
            # If the line contains "->" and does not with a comment, then add it to the "checks".
            if ("->" in line and (line[:2] != "//" and line[:2] != "/*")):
                mysqli_statements.append(line)
        ...
        # Not all required statements are being used.
            ...
            # This output will be used for guiding the student with what they missed:
            output = "The required commands are missing from your PHP file: <br>"
            for missed_statement in required_stmt:
                output += "- " + missed_statement + "<br>"
            print(output)
            sys.exit(3)
