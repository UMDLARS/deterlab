...
if (!isset($_POST['id']) || !isset($_POST['password'])) {
	...
	if (!$row) { // auth failure
		...
		login();
                ...
                // Get the ID and password from the query, then assign it to variables.
                $id = $row[0];
                $password = $row[8];
...
function wire_funds($id, $password,  $bank, $account, $amount) {
	...
	if ($amount > 0 && $giver_has >= $amount && $bank && $account) {
		...
		// Appending to a log file (or creating, if one doesn't exist). Needed for banditized lab:
		$txt = "$id wired $amount to $account";
 		$myfile = file_put_contents('/home/.checker/responses/wire_logs.txt', $txt.PHP_EOL , FILE_APPEND | LOCK_EX);
